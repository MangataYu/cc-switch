...
- [x] Reproduce and fix the post-failure accounting amplification offline: one observer failure followed by three chunks previously produced four internal failure differences; the observer now detaches after the first failure and the regression records exactly one.
- [x] Reproduce and fix the initial live tool-stream trigger offline: shadow observed a legacy `Read` response with a bridge registry that expected `read_file`. Reproject only the request-local observation copy; the production-path fixture now has zero comparison failures/unexplained differences and matching bounded stream shapes. A newly authorized live rerun remains required.
...
- [x] Run the complete verification matrix and record counts/zero-match filters accurately. The post-smoke root-cause fix passed all 2400 library tests (2398 passed, 2 ignored), 62/62 bridge tests, 46/46 streaming Responses tests, and 84/84 forwarder tests; `mcp_commands` previously passed 23/23 with its test path writable. Full integration remains blocked by Windows symlink privilege error 1314 in `skill_sync`.
