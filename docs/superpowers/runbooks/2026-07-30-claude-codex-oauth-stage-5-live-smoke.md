...
**Status:** Failed on 2026-07-30 with blocker `FixtureCoverageIncomplete`; rollout remains blocked. All required `shadow` tool fixtures pass. The completed `enabled` fixture rerun passed Read, Read-before-Edit, and Bash, but its Glob/Grep lifecycle was sequential. A fixed-port follow-up cleared the intervening client connection setup error and again produced one successful Glob plus one successful Grep, but in two assistant turns rather than one parallel batch. The earlier cleanup observation `IsolatedCredentialCopyChanged` remains recorded; both real credential files retained their baseline hashes in every later attempt. Optional MCP/Task remain unverified.

## 2026-07-30 fixed-port enabled parallel follow-up

- Setup correction: used a new isolated home, database, provider, and fixtures, with the isolated Claude Code `settings.json` written by takeover to fixed loopback port 15721. The Claude process was explicitly bound to that isolated configuration. The installed CC Switch remained stopped. The one-use binary held the real Codex access token only in process memory, retained the compiled `LiveNoRefreshGuard`, and neither copied nor created an OAuth store.
- Connection gate passed: the isolated proxy health check and both settings projections passed before the only Claude invocation. Unlike the preceding setup failure, this run reached two upstream requests and both returned HTTP 200. Safe diagnostics contained zero refresh or bridge-failure signals.
- Parallel gate failed: exactly one `Glob` and one `Grep` were emitted with two unique call identities, two matching successful result closures, no other tool, no result error, and no top-level error. However, they occupied two tool-bearing assistant turns; the first result arrived before the second tool use. The lifecycle was therefore sequential rather than the required same-turn parallel batch.
- Fail-fast and rollback boundary: no second prompt or visible-tool retry was issued. Because the parallel gate failed, the conditional no-tool rollback probe was not run. The disposable provider was set to explicit `legacy` before cleanup.
- Credential and cleanup boundary: both real credential files retained their supplied baseline SHA-256 hashes. The exact isolated process was stopped, no matching process remained, and the exact temporary directory containing the database, provider, fixtures, session records, and logs was deleted.
- Readiness remains `LiveSmokeStatus::Failed` with blocker `FixtureCoverageIncomplete`. The client setup blocker is cleared, but rollout still requires one actual same-assistant-turn Glob/Grep batch, both unique closures, and the subsequent explicit-legacy no-tool rollback probe.

## 2026-07-30 enabled parallel-only continuation attempt

- Scope and refresh boundary: this separately authorized attempt targeted only the remaining `enabled` Glob/Grep parallel batch and its continuation closure. The installed CC Switch remained stopped. A one-use binary read the current Codex access token from the real Codex auth file into process memory and compiled the forwarder refresh implementation down to a fixed `LiveNoRefreshGuard` error; no OAuth store was copied or created. The temporary source edits were restored immediately after the binary was copied, and the worktree excluding `.codegraph/` was clean before any isolated startup.
- Isolated setup: used a new temporary home, database, current `enabled` provider, loopback port, and two disposable fixtures. Setup-only application starts generated the new database and corrected the initially absent disposable Claude live-config file. They produced zero proxy requests and no model/tool traffic. The final isolated application and proxy health checks passed before the one allowed Claude Code invocation.
- Blocking result: the single restricted Claude Code invocation ended with safe reason code `ClientConnectionFailed`. Its UTF-16 JSONL contained 13 structurally parseable records and a top-level error, but zero `Glob`, zero `Grep`, zero other tool calls, zero tool results, and zero proxy/upstream requests. Consequently there were no tool identities or closures to validate, and the required same-assistant-turn condition was not formed. This is client/setup evidence only; it is not a bridge or upstream failure.
- Fail-fast boundary: no second prompt or visible-tool retry was issued. Because the parallel gate did not pass, the conditional explicit-legacy no-tool rollback probe was not run. Safe diagnostics contained zero refresh, HTTP 401, HTTP 403, or bridge-failure signals.
- Credential and cleanup boundary: before cleanup, the real CC Switch OAuth store and real Codex auth file retained their supplied baseline SHA-256 hashes. The exact isolated binary process was stopped, no matching process remained, and the exact temporary root containing the database, provider, fixtures, session records, and logs was deleted.
- Readiness remains `LiveSmokeStatus::Failed` with blocker `FixtureCoverageIncomplete`. The prior enabled Read, Read-before-Edit, and Bash evidence remains valid, but a later separately authorized attempt must still produce exactly one `Glob` and one `Grep` in one assistant turn before either result, close both unique identities exactly once, and then pass the explicit legacy no-tool rollback probe.

## 2026-07-30 enabled required-fixture rerun

- Refresh boundary: the installed CC Switch remained stopped. A temporary binary accepted the current access token only from process memory; its request path had no reachable refresh call. Safe application logs contained zero refresh signals, zero bridge failures, and zero error-level records. All 13 isolated streaming upstream requests returned HTTP 200 with no recorded request error.
- Read passed without retry: exactly one Read, one unique call identity, one matching successful result closure, no other tool, and exit 0. The requested fixture identity matched and the strict emitted arguments omitted the semantically empty `pages` field. An initial safe runner summary incorrectly reported zero tools because the installed PowerShell did not support the requested JSON parser depth option; the already-written isolated session record corrected the structural count without issuing another model request.
- Read-before-Edit passed: exactly one Read followed by its matching result, then exactly one Edit followed by its matching successful result. Both call identities were unique, both tool argument projections matched the disposable fixture, no tool was retried, and the replacement marker was present while the original marker was absent.
- Bash passed: exactly one Bash and one matching successful result closure, with no retry. The harmless disposable marker was created and matched.
- Parallel fixture remained incomplete: Glob and Grep each appeared exactly once with two unique identities, matching arguments, two matching successful results, and no retry. However, the safe lifecycle was `Glob -> result -> Grep -> result`, not one assistant message containing both tool calls before either result. Because raw upstream SSE was not retained, this run does not attribute the sequential choice to the model or the bridge and does not claim parallel coverage.
- Fail-fast boundary: the parallel fixture was not retried. Optional MCP/Task were not run. The provider was returned to explicit `legacy`, proxy/failover were disabled, and the exact isolated application and renderer processes were stopped by verified identity.
- Credential and cleanup boundary: the real CC Switch OAuth store and real Codex auth file retained their pre-run SHA-256 hashes, and no CC Switch process remained. The isolated OAuth copy did not retain its original file hash at cleanup. The no-refresh request guard and zero refresh log signals rule out the normal forwarder refresh path, but the isolated copy and raw logs were deleted before the hash mismatch was attributed, so the run records only `IsolatedCredentialCopyChanged` and does not speculate about its contents or cause. The exact temporary directory containing the database/provider copies, session records, logs, fixtures, and credential copy was deleted.
- Readiness remains `LiveSmokeStatus::Failed` with blocker `FixtureCoverageIncomplete`. A later explicitly authorized run must obtain an actual parallel enabled tool batch and matching continuation closure before rollout status can change.

## 2026-07-30 strict prepared-stream Read projection follow-up

- Refresh boundary: the installed CC Switch remained stopped. The isolated binary required an access token in process memory and had no reachable refresh call. The real CC Switch OAuth store and real Codex auth file retained their pre-run SHA-256 hashes throughout the attempt.
- Setup-only failures: two pre-request desktop startup attempts never opened the proxy and emitted no request. A later sandboxed-network attempt generated 21 client retries that all ended as local HTTP 502 `forward_failed` before any tool became visible; those rows were deleted from the isolated database before the authorized network-capable run and are not bridge evidence.
- Enabled Read evidence: with the isolated proxy health endpoint and renderer both returning HTTP 200, Claude Code emitted exactly one Read and one matching tool-result closure, no other tool, no tool retry, no top-level result error, and exit 0. The requested fixture identity matched, but the visible input still contained `pages` as an empty string and Claude Code returned `invalid_arguments`.
- Fail-fast boundary: the matrix stopped immediately after the visible Read error. Edit, Bash, parallel tools, continuations beyond the Read closure, and optional MCP/Task were not run.
- Corrected production diagnosis: enabled traffic uses `create_strict_anthropic_sse_stream_from_responses`, whose `PreparedCodexStream::tool_events` completed strict registry validation but serialized the original raw arguments. The earlier fix covered registry restoration and the compatibility core converter, not this strict emitter. A focused test through the real prepared-turn production converter was RED because its output retained empty Read `pages`, matching the live failure.
- Minimal fix: strict `tool_events` now serializes `RestoredToolCall.input`, which is produced only after original-schema validation and the narrow Read execution projection. Non-empty `pages`, offset, all other arguments, tool/call identity, ledger hashes, lifecycle/order, reasoning/signature, terminal completeness, and visible-tool retry safety remain strict. The focused test is GREEN, including fragmented logical argument boundaries.
- Verification: `claude_codex_bridge` 77 passed, `streaming_responses` 49 passed, and `forwarder` 84 passed. `cargo check`, `cargo fmt -- --check`, and `git diff --check` passed.
- Rollback and cleanup: the isolated provider was returned to explicit `legacy`, proxy/failover were disabled, the exact elevated application and renderer processes were stopped by verified identity, and the temporary directory containing the database/provider copies, raw prompts, events, arguments/results, logs, and fixtures was deleted. No CC Switch process remained.
- Readiness remains `LiveSmokeStatus::Failed` with blocker `FixtureCoverageIncomplete`. Another explicit live authorization is required to verify Read after the strict-emitter fix and, only if it passes, continue with Read-before-Edit, Bash, parallel tools, and remaining continuations.
